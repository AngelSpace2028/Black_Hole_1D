...
def move_bits_randomly(data, shift_min=1024, shift_max=(2**31 - 1)):  # Shift range from 1024 to 2^31 - 1
    """Moves bits from left to right and right to left by a random amount within a new range."""
    modified_data = bytearray()
    for byte in data:
        shift_amount = random.randint(shift_min, shift_max)  # Random shift value between 1024 and 2^31 - 1
        # Perform left shift
...
def find_best_iteration(input_filename, max_iterations):
    ...
    for iteration in range(max_iterations):
        ...
        # Alternate strategies based on the iteration number
        if iteration % 2 == 0:  # Even iterations - Strategy 1
            reversed_data = reverse_chunks_at_positions(file_data, chunk_size, positions)
            compressed_data = compress_with_paq(reversed_data, chunk_size, positions, file_size, 1)
            compression_ratio = len(compressed_data) / file_size
        else:  # Odd iterations - Strategy 2
            reversed_data = reverse_chunks_at_positions(file_data, chunk_size, positions)
            modified_data = add_random_bytes(reversed_data)
            compressed_data = compress_with_paq(modified_data, chunk_size, positions, file_size, 2)
            compression_ratio = len(compressed_data) / file_size
